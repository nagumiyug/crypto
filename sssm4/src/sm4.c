...
int sm4_make_enc_subkeys(const unsigned char key[SM4_KEY_SIZE], uint32_t encSubKeys[SM4_ROUNDS]) {
    ...
    for (int i = 0; i < 32; i++) {
        ...
        // SBox(t)
        uint32_t st = 0;
        st ^= ((uint32_t)SBox[(t >> 24) & 0x0FF] << 24) & 0x0FF000000;
        st ^= ((uint32_t)SBox[(t >> 16) & 0x0FF] << 16) & 0x0FF0000;
        st ^= ((uint32_t)SBox[(t >> 8) & 0x0FF] << 8) & 0x0FF00;
        st ^= (uint32_t)SBox[t & 0x0FF] & 0x0FF;
        // L'(SBox)
        uint32_t lt = st ^ ((st << 13) | (st >> 19)) ^ ((st << 23) | (st >> 9));
        uint32_t rk = K[0] ^ lt;
        // encSubKeys
        encSubKeys[i] = rk;
        // 下一轮
...
void sm4_encrypt_block(const unsigned char *input, const uint32_t encSubKeys[SM4_ROUNDS], unsigned char *output) {
    if (!input || !encSubKeys || !output) {
        return ;
    }
    // 输入分解 
    uint32_t X[4] = {0};
    X[0] = ((uint32_t)input[0] << 24) | ((uint32_t)input[1] << 16) | ((uint32_t)input[2] << 8) | (uint32_t)input[3];
    X[1] = ((uint32_t)input[4] << 24) | ((uint32_t)input[5] << 16) | ((uint32_t)input[6] << 8) | (uint32_t)input[7];
    X[2] = ((uint32_t)input[8] << 24) | ((uint32_t)input[9] << 16) | ((uint32_t)input[10] << 8) | (uint32_t)input[11];
    X[3] = ((uint32_t)input[12] << 24) | ((uint32_t)input[13] << 16) | ((uint32_t)input[14] << 8) | (uint32_t)input[15];
    
    for (int i = 0; i < 32; i++) {
        uint32_t rk = encSubKeys[i];
        uint32_t lx = X[1] ^ X[2] ^ X[3] ^ rk;
        uint32_t Xre = X[0];
        Xre ^= Table0[(lx >> 24) & 0x0FF];
        Xre ^= Table1[(lx >> 16) & 0x0FF];
        Xre ^= Table2[(lx >> 8) & 0x0FF];
        Xre ^= Table3[lx & 0x0FF];
        X[0] = X[1];
        X[1] = X[2];
        X[2] = X[3];
    ...
    output[0] = (X[3] >> 24) & 0xFF; output[1] = (X[3] >> 16) & 0xFF; output[2] = (X[3] >> 8) & 0xFF; output[3] = X[3] & 0xFF;
    output[4] = (X[2] >> 24) & 0xFF; output[5] = (X[2] >> 16) & 0xFF; output[6] = (X[2] >> 8) & 0xFF; output[7] = X[2] & 0xFF;
    output[8] = (X[1] >> 24) & 0xFF; output[9] = (X[1] >> 16) & 0xFF; output[10] = (X[1] >> 8) & 0xFF; output[11] = X[1] & 0xFF;
    output[12] = (X[0] >> 24) & 0xFF; output[13] = (X[0] >> 16) & 0xFF; output[14] = (X[0] >> 8) & 0xFF; output[15] = X[0] & 0xFF;
}
